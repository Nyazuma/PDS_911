...
public class Gui implements ActionListener{
	...
	public void actionPerformed(ActionEvent event) {
		...
		if(event.getSource().equals(ValiderButton)) {
			if(!(gisSession.connection(identifiantTxt.getText(), String.valueOf(passwordField.getPassword())))) {
				failConnexion();
			}
			else
			{
		...
		System.out.println("B " + event.toString());
		if(event.getSource().equals(btnAjouter)) {
			System.out.println("C");
			gisSession.addObject(listCapteur.getSelectedItem().toString());
			lblNewLabel.setText(Integer.toString(gisSession.nbObject())); 
